...
// Row type describing your watchtime data shape
...
const StatusAndOnboardingLegend = () => (
  <div className="flex items-center gap-x-6 gap-y-2 mb-4 flex-wrap p-4 bg-card rounded-lg border">
    <div className="flex items-center gap-2 text-sm">
      <Person className="h-4 w-4 text-blue-500" /> <span>Manual Onboard</span>
    </div>
    <div className="flex items-center gap-2 text-sm">
      <Person className="h-4 w-4 text-orange-500" /> <span>Self Onboard</span>
    </div>
    <div className="flex items-center gap-2 text-sm">
      <School className="h-4 w-4 text-green-500" /> <span>Student</span>
    </div>
    <div className="flex items-center gap-2 text-sm">
      <AccountCircle className="h-4 w-4 text-blue-500" /> <span>Teacher</span>
    </div>
  </div>
);
...
const DataSummary = ({ data }: { data: WatchtimeRow[] }) => {
  const stats = useMemo(() => ({
    ...
    totalCourseWT: data.reduce((sum, item) => sum + (item.total_watched_minutes_q1 || 0), 0),
    totalHomeworkWT: data.reduce((sum, item) => sum + (item.total_watched_lesson_duration_in_minutes || 0), 0),
  }), [data]);
...
const DataTableWatchtime = () => {
  ...
  const [visibleRows, setVisibleRows] = useState<WatchtimeRow[]>([]);
  ...
  const columns: GridColDef[] = React.useMemo(() => [
    { field: 'email', headerName: 'Email', flex: 3 },
    {
      ...
      flex: 1,
      renderCell: (params: GridRenderCellParams<WatchtimeRow>) => {
        const key = params.value as WatchtimeRow['user_type'];
        const icon = userTypeIconMap[key];
        return icon ? <Tooltip title={key ?? ''}>{icon}</Tooltip> : null;
      }
    },
    {
      field: 'user_join',
      ...
      flex: 1,
      renderCell: (params: GridRenderCellParams<WatchtimeRow>) => {
        const key = params.value as WatchtimeRow['user_join'];
        const icon = onboardingIconMap[key];
        return icon ? <Tooltip title={key ?? ''}>{icon}</Tooltip> : null;
      }
    },
